...
layout(location = 5) uniform int PickedID;
...
vec4 compute_pick_output(int ObjectID) {
	vec4 pick ;
	pick.x = ObjectID;
	pick.y = 0;
	pick.z = 0;
...
void main(void)
{
	if (pass == 0)
	{
		...
		pick = compute_pick_output(ObjectID);
...
vec4 myeffect()
{
	...
	vec4 originalColor = texelFetch(fbo_tex, ivec2(gl_FragCoord), 0);


	// Invert the colors
	vec4 invertedColor = vec4(1.0) - originalColor;
